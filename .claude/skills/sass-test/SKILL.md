...
|---|---|---|
| smoke (`test/smoke.scss`) | the mixin evaluates | 49/49 mixins |
| snapshot (`meta/` examples) | its output cannot change unnoticed | 71/71 members |
| rejection (`meta/` rejects) | bad input is refused, with the library's own message | 42/71 |
| **sass-true spec** | **the CSS is correct** | **9/71** |
