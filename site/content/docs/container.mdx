...
<Example caption="A named container. The name is what <code>container-query</code> targets when several containers are nested.">
```scss
.card {
...
<Example caption="Without a name. Queries with no <code>$name</code> resolve to the nearest container ancestor.">
```scss
.panel {
  @include container;
}
