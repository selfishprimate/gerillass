...
<Arguments of="background-dots" footnote="Using two separate color values together is only possible when the <code>$diagonal</code> option is true. See the examples for more.">
...
<Example caption="Let's play with the <code>$color</code> and <code>$size</code> arguments.">
```scss
.element{
  @include background-dots(
    $color: pink,
...
<Example caption="You can add a second color value as well. Note that the second color only works when the <code>$diagonal</code> argument is <code>true</code>, and the two colors must be separated by a space.">
```scss
.element{
  @include background-dots(
    $color: red orange,
...
<Example caption="Set the <code>$diagonal</code> argument to <code>false</code> to make the dots look more linear.">
```scss
.element{
  @include background-dots(
    $color: red,
