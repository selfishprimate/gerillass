...
<Example caption="Simply call the mixin without passing any arguments. The default <code>$gutter</code> value is <code>30px</code>.">
```scss
.main-container{
  @include adaptive;
}
...
<Example caption="Try passing a value with an <code>em</code> unit. You can use any length unit here, so <code>px</code>, <code>em</code>, <code>rem</code>, and percentages all work.">
```scss
.main-container{
