...
<Arguments of="validateLength">
  <Argument name="$value" type="number (with unit)">Accepts a length with a unit, or auto, inherit, initial, 0. <code>null</code> is returned as it is, without a warning, so a caller can skip a value.</Argument>
</Arguments>
