...
page_description: "How to install Gerillass and load it in Vite, webpack, Next.js, Angular, Gulp, Grunt, Rails, Jekyll or Dart Sass, plus the three ways to call a mixin."
page_keywords: "Install Gerillass, Sass library installation, Sass mixins library, use Sass with Vite, sass-loader, Next.js sassOptions, Dart Sass pkg importer, npm install gerillass, Gerillass gem, Sass in Rails, dartsass-rails, Jekyll Sass"
---
...
Gerillass is also a gem. It installs the same Sass files and has no runtime
dependencies, because a Rails app already brings its own Dart Sass.

```bash
bundle add gerillass
```

With `dartsass-rails` on Propshaft, the Rails 8 default, or `dartsass-sprockets`
on Sprockets, there is nothing to configure. The gem hands the library's folder
to Sass, and none of its files end up in `public/assets`. Then, in
...
The library needs Dart Sass. The LibSass-based `sass-rails` and `sassc-rails`
cannot compile it.
...
Put the gem in the plugins group of your `Gemfile`:
...
Without Rails or Jekyll, pass the library's folder to `sass-embedded` yourself:
...
`node_modules/gerillass/`, or, in a Ruby project, out of the gem's folder, which
`bundle info gerillass --path` prints.
