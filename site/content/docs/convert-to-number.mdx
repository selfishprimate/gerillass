---
title: "Convert To Number"
page_title: "Convert To Number Sass Function"
page_description: "The convertToNumber Sass function parses a string of digits into a number, which is what you need when a value arrives from interpolation as text."
page_keywords: "Gerillass convertToNumber, Sass convertToNumber, Sass Functions, SCSS Functions, Sass Utility Functions, Gerillass Utilities"
---

# Convert To Number

<Member name="convertToNumber">

Parses a string of digits into a number.

</Member>

## Arguments

<Arguments of="convertToNumber">
  <Argument name="$value" type="string">Accepts a string containing only digits.</Argument>
</Arguments>

## Examples

<Example caption="A numeric string turned into a number you can do arithmetic with.">
```scss
.column {
  width: convertToNumber("50") * 1%;
}
```
</Example>

## What it refuses

Arguments are checked, so a wrong value stops the build with a message instead of quietly producing the wrong CSS.

```scss
.column {
  width: convertToNumber(50) * 1%;
}
```
```text
Error: `50` is not a valid $value for `convertToNumber`. Pass a string made only of digits, such as `"42"`.
```

## Related Links

- [Clear Unit](/docs/clear-unit)
- [CSS number](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/number)
- [CSS Values and Units](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Values_and_units)
