...
<Arguments of="background-stripes" footnote="Multiple color values must be separated by a space (e.g. <code>$color: red blue green</code>).">
...
<Example caption="Let's pass a <code>$color</code> value to it!">
```scss
.element{
  @include background-stripes(
    $color: pink
...
<Example caption="Now let's add more colors and change the <code>$thickness</code> and <code>$rotation</code> values. <strong>Important:</strong> Don't forget that multiple color values must be separated by a space!">
```scss
.element{
  @include background-stripes(
...
<Example caption="Add an image to make your design look more alive! <strong>Just do not forget to use quotes when you pass the URL of an image.</strong>">
```scss
.element{
  @include background-stripes(
    $color: rgba(blue, 0.4),
