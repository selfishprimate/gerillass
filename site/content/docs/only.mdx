...
```
```text
Error: `10px` is not a valid position for `only`: a position is a whole number without a unit, and `:nth-of-type(10px)` is an invalid selector, so the browser would drop the rule. Pass a number such as `2`, or `-2` to count from the end.
```
