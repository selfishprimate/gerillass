...
<Arguments of="only" footnote="<strong>Important:</strong> Passing multiple arguments only works with numeric values, and the values must be separated by commas.">
...
<Example caption="Let's start with the <strong>first</strong> one!">
```scss
.list-wrapper{
  .list-item{
    @include only(first) {
...
<Example caption="Now, let's try to get the <strong>last item</strong> in the list.">
```scss
.list-wrapper{
  .list-item{
    @include only(last) {
...
<Example caption="Now, let's target the <strong>second</strong> item in the list.">
```scss
.list-wrapper{
  .list-item{
    @include only(2) {
...
<Example caption="Now suppose you don't know how many items will appear in the list, and <strong>you want to target only the third item from the end</strong>. How can you do that? It's surprisingly easy!">
```scss
.list-wrapper {
  .list-item {
    @include only(-3) {
