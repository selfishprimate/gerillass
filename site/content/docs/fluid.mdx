---
title: "Fluid"
page_title: "Fluid Sass Function"
page_description: "The fluid Sass function returns a clamp() value that grows with the viewport between two widths, then stops. It keeps a rem term so text zoom still works."
page_keywords: "Fluid Typography Sass, CSS clamp Function, Fluid Type Scale, Sass clamp, Responsive Font Size, WCAG 1.4.4 Resize Text, vw Font Size Accessibility, Fluid Spacing CSS"
---

# Fluid

<Member name="fluid">

A value that grows with the viewport between two widths and then stops. `fluid(24px, 48px)` is `24px` on a small screen, `48px` on a large one, and a straight line between the two.

<Hint kind="danger">

**This is why the function exists.** A fluid value written by hand usually comes out as pure `vw`, and a `vw`-only value ignores browser text zoom: raising the root font size moved this function's value from 20.83px to 34.17px, while the `vw`-only one stayed at 19.74px. That failure is [WCAG 1.4.4](https://www.w3.org/WAI/WCAG21/Understanding/resize-text.html); the preferred value here keeps a `rem` term so it does not.

</Hint>

<Hint kind="info">

**It is a function, not a mixin, so there is no `@include`.** The hard part is the value, and a value belongs to any property. `font-size` is the obvious one, but `padding`, `gap` and `margin` take it just as well.

</Hint>

</Member>

## How the value is built

The preferred value is the line through `(min-viewport, min)` and `(max-viewport, max)`.

```nix
slope     = (max - min) / (max-viewport - min-viewport)
intercept = min - slope * min-viewport
preferred = intercept + slope * 100vw
```

Checked against hand arithmetic at a real 900px viewport: the floor came out at 16.00px against 16.00 expected, the ceiling at 24.00 against 24.00, and an interpolated value at 19.52 against 19.52.

## Arguments

<Arguments of="fluid" footnote="Lengths must be px or rem. Other units are refused, because the function has to convert between them to build the line.">
  <Argument name="$min" type="number (with unit)">Accepts a length in px or rem, the value at <code>$min-viewport</code>.</Argument>
  <Argument name="$max" type="number (with unit)">Accepts a length in px or rem, the value at <code>$max-viewport</code>, no smaller than <code>$min</code>.</Argument>
  <Argument name="$min-viewport (320px)" type="number (with unit)">Accepts a length in px or rem, the width below which the value stops shrinking.</Argument>
  <Argument name="$max-viewport (1280px)" type="number (with unit)">Accepts a length in px or rem, larger than <code>$min-viewport</code>.</Argument>
</Arguments>

## Examples

<Example caption="A heading that scales between the two default viewport widths.">
```scss
.title {
  font-size: fluid(24px, 48px);
}
```
</Example>

<Example caption="Your own viewport range, and values given in `rem`.">
```scss
.title {
  font-size: fluid(1rem, 3rem, 320px, 1200px);
}
```
</Example>

<Example caption="Not only for type. Two calls in one shorthand give a padding that scales on both axes.">
```scss
.section {
  padding: fluid(16px, 64px) fluid(8px, 40px);
}
```
</Example>

<Example caption="A grid gap that opens up on wider screens.">
```scss
.stack {
  gap: fluid(0.5rem, 2rem);
}
```
</Example>

## What it refuses

Arguments are checked, so a wrong value stops the build with a message instead of quietly producing a value that never changes.

```scss
.title {
  font-size: fluid(16px, 24px, 1280px, 320px);
}
```
```text
Error: `fluid` needs $min-viewport to be smaller than $max-viewport, and was given 1280px and 320px.
```

```scss
.title {
  font-size: fluid(40px, 20px);
}
```
```text
Error: `fluid` needs $min to be no larger than $max, and was given 40px and 20px. clamp() would return the floor at every width, so the value would never grow.
```

```scss
.title {
  font-size: fluid(16, 24);
}
```
```text
Error: `16` is not a valid $min for `fluid`. Pass a length in px or rem.
```

```scss
.title {
  font-size: fluid(1em, 2em);
}
```
```text
Error: `1em` is not a valid $min for `fluid`. Pass a length in px or rem, not em.
```

## Related Links
* [clamp()](https://developer.mozilla.org/en-US/docs/Web/CSS/clamp)
* [WCAG 1.4.4 Resize Text](https://www.w3.org/WAI/WCAG21/Understanding/resize-text.html)

