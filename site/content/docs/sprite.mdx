...
<Example caption="Or you can apply it to multiple elements, with a different image for each one selected from a single sprite sheet. **Suppose you have markup like the one below, and you want to assign a different background image to each element from a single sprite sheet.**" listing="<div class=&quot;sprite-element first&quot;></div>
