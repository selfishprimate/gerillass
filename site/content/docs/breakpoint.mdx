...
**One width on its own is a single pixel.** `breakpoint(320px)` compiles to `@media (width: 320px)`, so the styles apply only when the viewport is exactly 320px wide, and at every other width they do nothing. Since 2.2.0 the mixin prints a warning for that form, and 3.0.0 will refuse it. To style from a width upwards pass `min`, and for that exact width pass `only`.
