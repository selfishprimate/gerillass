...
<Example caption="It is important to use markup similar to the example below. The containing element does not have to be an <code>&lt;h1&gt;</code> tag, so feel free to change it depending on the SEO strategy you use on your site.">
```scss
.brand-logo{
  @include brand-logo(
    $width: 100px,
...
<Example caption="As mentioned before, the <code>$image-url</code> argument is optional. With that in mind, let's specify the logo image using the selected element's style attribute.">
```scss
.brand-logo{
  @include brand-logo(
    $width: 100px,
