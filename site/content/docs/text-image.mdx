...
<Arguments of="text-image" footnote="<strong>Important:</strong> Note that the URL of the image must be passed in quotation marks.">
...
<Example caption="The <code>$image</code> argument is optional. This is especially useful when the image is loaded from the markup, as in the example below.">
```scss
.element{
  @include text-image;
}
