...
<Arguments of="background-image" footnote="Use <code>null</code> if you want to skip an argument. See [the examples](#examples) for more.">
...
<Example caption="Now let's apply a color filter to it by passing a color value for the <code>$filter-color</code> argument.">
```scss
.element{
...
<Example caption="Now let's pass multiple color values for <code>$filter-color</code> to make the background image look even more interesting.">
```scss
.element{
...
<Example caption="Now let's try the <code>$filter-direction</code> option. While we do, let's use sharper color transitions to see the effect clearly.">
```scss
.element{
...
<Example caption="There will be times when you want to <strong>add a background image to an element using the style attribute</strong>, like in the example below. <strong>In those cases, just use <code>null</code> to skip the <code>$image-url</code> argument.</strong>">
```scss
.element{
...
<Example caption="You are probably wondering why those <code>position: relative</code> and <code>z-index: 1</code> style rules are applied to the direct children of the selected element. <strong>These rules exist to prevent the color filter from covering the direct children when you add the background image to an element using the style attribute.</strong>
