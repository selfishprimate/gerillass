...
<Arguments of="container-query" footnote="$name is passed as a keyword rather than by position, because <code>container-query(&quot;card&quot;, &quot;medium&quot;)</code> could not be told apart from <code>container-query(&quot;min&quot;, &quot;medium&quot;)</code>.">
...
<Example caption="A range, written either with <code>between</code> or as two sizes. Both produce the same rule.">
```scss
.title {
...
<Example caption="A predefined breakpoint name works too, resolved through <code>$map-for-breakpoints</code>.">
```scss
.title {
