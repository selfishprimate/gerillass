...
A size on its own matches that exact width and no other, `(width: 400px)`, which is a single pixel. The mixin prints a warning for it since 2.2.0, and 3.0.0 will refuse it. Write `only` when that is what you mean.
