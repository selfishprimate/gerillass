...
- `$style: line` draws `isometric` as the grid the cubes sit on, three families of straight lines, over the same cubes shaded from the line's own colour: the shading is what makes them read as solid, since the grid holds every edge of the tiling and a cube is only the two thirds of them that bound a rhombus. No other pattern has one. A wireframe of the cubes alone, or a hexagon's or a brick's outline, needs its lines broken along their own direction, which a gradient cannot do: stacking the filled pattern over an offset copy hides the lower one, since the faces tile the plane with no gap, narrowing each conic wedge opens the spokes inside a hexagon and leaves its outline shut, and thin conic spokes give lines that thicken as they leave the centre of the tile.
- A lined `isometric` leaves gaps, so it takes an `$image` under it; the filled one refuses the argument, since the faces would hide it.
