...
- `$style: line` draws `isometric` as the grid the cubes sit on, three families of straight lines, and no other pattern has one. A hexagon's or a brick's outline needs its lines broken along their own direction, which a gradient cannot do: stacking the filled pattern over an offset copy hides the lower one, since the faces tile the plane with no gap, and thin conic spokes give lines that thicken as they leave the centre of each tile.
