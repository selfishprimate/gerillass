...
- The gutter is zero by default since 4.0.0, so the container is exactly the breakpoint wide. It was 30px, which made every container 60px narrower than the width it was named after, and a zero gutter is not written as a subtraction at all: `max-width: 576px` rather than `calc(576px - 0px * 2)`. Measured in Chrome 152, Firefox 156 and Safari 26.6.2, the two forms compute to the same 576px.
- A breakpoint of zero is skipped by its value since 4.0.0. It used to be dropped by name, `map.remove($map, "xsmall")`, and `$map-for-breakpoints` is `!default`: a project naming its own breakpoints kept the zero entry, and the mixin wrote `@media (min-width: 0)` with a `max-width` calculation in it. Measured in Chrome 152, Firefox 156 and Safari 26.6.2, that calculation is invalid with a unitless key and the declaration is dropped; written as `0px` with the old 30px gutter it computed to -60px, the browser clamped it to 0, and the container was 0 wide at every size.
...
- The filter layer carries `border-radius: inherit`, so it follows the element's corners. An absolutely positioned box does not inherit a radius and the element does not clip it, so on a rounded card the filter used to paint square corners over it; measured in Chrome 152 and Firefox 156, and not captured in Safari.
...
- A diagonal hard edge is drawn with stair steps in Firefox 156 and smoothly in Chrome 152 and Safari 26.6.2, which shows in `zigzag` and `triangles`. Feathering the stop by half a pixel was measured and did not change it.
...
- The default colours are `rgb(from currentColor r g b / …)` rather than `color-mix()`. Measured in Firefox 156, a `color-mix()` inside a repeating gradient is rasterised near the origin and not tiled, so a band of one or two pixels disappears over most of the box and `stripes` with no colour drew two faint lines; the same geometry with the relative form, with an rgba literal and with a plain `currentColor` drew the full field in Chrome 152, Firefox 156 and Safari 26.6.2.
- `triangles` takes a $direction rather than an angle. The shape is a quarter of the tile cut by a conic gradient, and only the odd multiples of 45 degrees cut it as a triangle: measured at twelve angles, 0, 90, 180 and 270 draw a square and 15, 30, 60, 75 and 120 a lopsided quadrilateral. The four names are the four rotations that work.
- A vignette eases from the middle out in four stops and reaches the corners by default. Two stops with a clear centre draw a visible oval halo where the darkening starts, which is what the first version did, and an ellipse that stops inside the box draws its own edge; both were rendered beside the eased version in Chrome 152, Firefox 156 and Safari 26.6.2.
...
- `true` fills the gap between the text and the shadow with one layer per step. Since 4.0.0 the default step is one pixel, and a fortieth of the distance for a unit only the browser resolves. Until then it was one of the distance's own unit, which is a pixel in `px` and nothing like it anywhere else: `2rem` drew two copies eleven pixels apart and `1cm` drew one, so a filled shadow in any unit but the pixel was a row of ghosts. Sass converts the absolute units, so `pt`, `cm` and `in` take the pixel too; `rem`, `em`, `ch` and the viewport units cannot be converted, and a fortieth is the rule `long-shadow` settled on. Pass `$step` for anything else, in the same unit as the distance.
...
|---|---|
| `adaptive($gutter: 0)` | Centred container whose max-width steps up at every breakpoint, full width below the first. |
...
| `background-pattern($kind: dots, $params...)` | Seventeen background patterns drawn with gradients alone: fourteen that tile, from dots and stripes to houndstooth and harlequin, and glow, vignette and mesh, which light the whole box. |
