...
7. ~~**`glass`** and **`edge-fade`**~~ **both done for 4.0.0**, on the `glass` and `edge-fade` branches. `glass` is the `@supports` fallback and the prefixed property around one `backdrop-filter`; `edge-fade` is a four stop gradient, `mask-composite: intersect` for two axes, and a second rule under `:dir(rtl)` so `start` and `end` follow the writing direction. Measured in all three browsers: the unprefixed properties and `:dir()` are everywhere, `-webkit-mask-composite` is Chrome's and Safari's alone, and scroll-driven timelines, which a fade that knows whether there is content past the edge would need, are missing in Firefox 156.
