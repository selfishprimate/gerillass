...
2. ~~**`sidebar($side-width, $content-min, $gap, $side)`**~~ **done for 4.0.0 on the `auto-grid` branch**, as `sidebar($side, $gap, $threshold, $side-on)`. What it is for was measured rather than argued: with the window at 1024px, so a 768px media query never fires, a grid with that query left the content at 356px in a 700px box, 216px in a 560px one and 60px in a 400px one, overflowing by 4px, while the mixin stacked in all three, in Chrome 152, Firefox 156 and Safari 26.6.2. The query reads the window and the layout lives in a box.
