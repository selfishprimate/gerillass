...
4. ~~**`presence($open, $duration, $backdrop)`**~~ **done for 4.0.0 on the `auto-grid` branch**, as `presence($open, $duration, $move, $scale, $backdrop, $easing)`. The `[measure]` is answered: **Firefox 156 does not run the exit**, on a popover, a modal dialog or a class-toggled element, although it answers true for `CSS.supports("transition-behavior", "allow-discrete")`; Chrome 152 and Safari 26.6.2 animate both ways. The two silent failures were measured as well, on all three shapes: without `@starting-style` nothing animates in, in any browser, and without `allow-discrete` on `display` nothing animates out, in any browser. `overlay` is Chrome's property alone and the measurement did not show what it buys, so it is written on the strength of what it is for rather than a result here.
